...
func SendMessage(ctx context.Context, message string, sender string, receiver string, database *db.MongoCollections) (*model.Message, error) {
	//check if receiver is exist or not
	...
	_, err := auth.GetUser(database.Users, ctx, receiver)
	if err != nil {
